<?php
    ...
    if (isset($_POST['submit'])) {
      ...
      if (empty($_POST['firstname']) || empty($_POST['lastname']) || empty($_POST['username']) || empty($_POST['password_1']) || empty($_POST['password_2'])){
        echo "please fill the gaps";
      }
      else if ($password_1==$password_2) {

        $password = md5($password_1);
        $sql= "INSERT INTO gamers1(firstname,lastname,username,email,password)
        VALUES('$firstname','$lastname','$username','$email','$password')";
        mysqli_query($db,$sql);
        $_SESSION['username']=$username;
        header("location: home.html");
      }else {
      //  $_SESSION['message']="do not match";
